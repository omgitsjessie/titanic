...
print(rpart.tree)
#Prune tree
# pruned.tree <- prune(rpart.tree, cp = 0)
...
#2. There are 177 missing Age points.  Predict age from Training Set.
# ageData <- train %>% filter(!is.na(Age))
# noAgeData <- train %>% filter(is.na(Age))
# ageModel <- glm(Age ~ Survived + Pclass + SibSp, 
#                 data=ageData)
...
write.csv(submitTest, file = "SubmittedPredictions.csv", row.names = F)
